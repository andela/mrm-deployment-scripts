...
create_converge_user() {
  if ! id -u converge; then
    sudo useradd -m -s /bin/bash converge
  fi
}
...
install_redis(){
  if ! which redis-cli; then
    ...
    sudo chgrp -R converge  /usr/local # make converge the group that /usr/local belongs to
